...
def check_guide(path: Path, installed_version: str, keep_scratch: bool = False) -> GuideReport:
    ...
    # Build the block-id -> output fence map, and validate FR-050: every
    # fence carrying sicario-block must also carry a recognized marker.
    ...
    for fence in fences:
        ...
        if block_id is None:
            ...
            # reported as unpaired either, because unpaired-verified
            # detection below only walks output_blocks. That is a silent
            # hole, not a pass: flag it as its own structural error.
            if marker == "verified":
                report.structural_errors.append(
        ...
        # illustrative fence is silently dropped and never diffed even
        # though it counts as paired — last-wins with no detection. Both
        # locations must be named so a guide author can tell which one to
        # rename.
        ...
        for key in normalize_attr.split(","):
            ...
            if key and key not in KNOWN_NORMALIZE_KEYS:
                ...
                # (neither applied nor reported) — make the module's own
                # claim about that true.
                report.structural_errors.append(
        ...
        if stream_attr is not None and stream_attr not in KNOWN_STREAMS:
            # F11: anything other than exactly "stdout" fell through to
            # stderr silently.
            report.structural_errors.append(
                f"{path}:{fence.line_no}: block {block_id!r} declares "
                f"invalid sicario-stream={stream_attr!r} (must be exactly "
                f"'stdout' or 'stderr')"
            )
        output_blocks[block_id] = fence

    paired_block_ids = set()
    ...
    try:
        ...
        # F3: the scratch root is the only place any fence may leave a
        # trace. Resolved once so every containment check below compares
        # against the same, symlink-free root (macOS's /tmp -> /private/tmp
        # would otherwise make a same-directory path look "outside").
        ...
        for idx, fence in enumerate(fences):
            ...
            if write_path:
                ...
                if not _is_contained(target, scratch_root_resolved):
                    # F3(b): a sicario-write destination that resolves
                    # outside scratch (``../x``, or an absolute path) is
                    # refused rather than written — the guide fails instead
                    # of leaving a file somewhere it should never reach.
                    report.structural_errors.append(
                        f"{path}:{fence.line_no}: sicario-write={write_path!r} "
                        f"resolves to {target} which is outside the scratch "
                        f"root ({scratch_root_resolved}) — refusing to write"
                    )
                    aborted = True
                    break
            ...
            if not _is_contained(new_cwd_resolved, scratch_root_resolved):
                ...
                # actual repository checkout — must not be allowed to keep
                # running there; every later sicario-write in this guide
                # would inherit that escaped cwd as its base.
                report.structural_errors.append(
                    f"{path}:{fence.line_no}: fence's working directory "
                    f"left the scratch root (now {new_cwd_resolved}, root "
                    f"is {scratch_root_resolved}) — refusing to continue"
                )
                aborted = True
                break
            cwd = new_cwd_resolved

            if cmd_attr == "setup":
                if result.returncode != 0:
                    # F10: a failing setup fence used to be silent — later
                    # verified fences would then run against whatever
                    # partial state setup left behind and fail with a
                    # confusing diff instead of the real, upstream cause.
                    report.structural_errors.append(
                        f"{path}:{fence.line_no}: sicario-cmd=setup fence "
                        f"exited {result.returncode} (state-prep failure) — "
                        f"stderr:\n{result.stderr}"
                    )
                    aborted = True
                    break
            ...
            # Non-setup (paired) sicario-cmd fences deliberately do NOT
            # check returncode: many guides pair a fence that exits
            # non-zero by design — a staged gate failure is itself the
            # pedagogy being demonstrated. The quoted output block —
            # including any quoted `echo $?` line — is the contract those
            # fences are verified against, not the process exit code.

            # Group the targeted blocks by stream: several output blocks can
            # come from ONE execution (a `verify` block on stdout plus a
            # separate `echo $?` block also on stdout; or a `--format json`
            # document on stdout paired with its stderr summary line as a
            # second block). Each stream's blocks are compared as a single
            # concatenation, in the order they are listed, against that
            # stream's full captured text.
            ...
            for stream, block_ids in by_stream.items():
                ...
                normalize_keys = []
                for b in block_ids:
                    normalize_keys.extend(
                        output_blocks[b].attrs.get("sicario-normalize", "").split(",")
                    )
                ...
                if observed_n.rstrip("\n") == quoted_n.rstrip("\n"):
                    ...
                    report.verified_failed.append(
                        Mismatch(
                            block_id=",".join(block_ids), line_no=first_fence.line_no, diff=diff
                        )
                    )
        ...
        if not aborted:
            for block_id, fence in output_blocks.items():
                if (
                    fence.attrs.get("sicario-output") == "illustrative"
                    and block_id not in paired_block_ids
                ):
                    report.illustrative_skipped += 1
    finally:
        if not keep_scratch:
